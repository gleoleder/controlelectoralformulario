...
function obtenerGanador(rec,datos){
  ...
  let mx=0,g=null;
  Object.entries(datos).forEach(([p,v])=>{if(v>mx){mx=v;g=p}});
  if(!g) return null;
...
function renderLeyenda(p){
  ...
  let h='<div class="legend-title">Ganador por recinto</div>';
...
function calcularResumenMunicipal(muni,dep){
  ...
  const tot={};let tv=0,cd=0;
  recs.forEach(r=>{const d=resultadosData[r.c];if(!d)return;cd++;Object.entries(d).forEach(([p,v])=>{tot[p]=(tot[p]||0)+v;tv+=v})});
...
function abrirModal(rec){
  ...
  if(!datos||!Object.keys(datos).length){
    ...
    let tv=0;Object.values(datos).forEach(v=>tv+=v);
    const items=Object.entries(datos).map(([p,v])=>{const c=ca.find(x=>x.partido===p);return{partido:p,votos:v,nombre:c?.nombre||p,color:c?.color||'#6B7280',pct:tv>0?(v/tv*100):0}}).sort((a,b)=>b.votos-a.votos);
    const g=items[0],mx=Math.max(...items.map(i=>i.votos));
    html+=`<div class="m-stats"><div class="m-stat"><span class="m-stat-v">${tv.toLocaleString()}</span><span class="m-stat-l">votos</span></div><div class="m-stat"><span class="m-stat-v">${rec.ms||1}</span><span class="m-stat-l">mesas</span></div><div class="m-stat"><span class="m-stat-v">${rec.h||0}</span><span class="m-stat-l">hab.</span></div><div class="m-stat winner" style="border-color:${g.color}"><span class="m-stat-v" style="color:${g.color}">🏆 ${g.partido}</span><span class="m-stat-l">${g.pct.toFixed(1)}%</span></div></div>`;
    html+=`<div class="m-chart">`;
    items.forEach(i=>{const w=mx>0?Math.max((i.votos/mx*100),3):3;html+=`<div class="m-row"><span class="m-sigla">${i.partido}</span><div class="m-bar-bg"><div class="m-bar-fill" style="width:${w}%;background:${i.color}"></div></div><span class="m-vnum">${i.votos}</span><span class="m-vpct" style="color:${i===g?i.color:''}">${i.pct.toFixed(1)}%</span></div>`});
    html+=`</div>`;
  }
