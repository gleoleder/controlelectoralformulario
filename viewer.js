...
async function cargarTodosDatos(){
  ...
  try{
    ...
    fotosData={};
    fot.forEach(f=>{const c=f.codigo,u=f.url_foto,m=f.mesa||'Mesa 1';if(!c||!u)return;if(!fotosData[c])fotosData[c]={};if(!fotosData[c][m])fotosData[c][m]=[];if(!fotosData[c][m].includes(u))fotosData[c][m].push(u)});
    candidatosData={};
...
function abrirModal(rec){
  ...
  if(fotos&&Object.keys(fotos).length>0){
    ...
    Object.entries(fotos).forEach(([mesa,urls])=>{
      const url=urls[0]; // Only first photo per mesa
      const th=getDriveThumbnail(url),op=getDriveOpenUrl(url);
      html+=`<div class="m-foto" onclick="window.open('${op}','_blank')"><img src="${th}" alt="${mesa}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="m-foto-fallback" style="display:none">📷</div><span class="m-foto-label">${mesa}</span></div>`;
    });
    html+=`</div>`;
  }
