...
// Versión funcional con Google Sheets API
// ═══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════
// GOOGLE SHEETS API - FUNCIONES DE LECTURA
// ══════════════════════════════════════════════════════════════
...
async function agregarFilasASheet(nombreHoja, filas) {
  try {
...
async function cargarCandidatos() {
  try {
    ...
    hideLoading();
    showToast('⚠️ No se pudieron cargar candidatos. Usando predeterminados.', 'warning');
    return false;
  }
}
...
function renderizarFormulario() {
  ...
  html += '<strong>💡 Formato Google Drive:</strong><br>';
...
async function guardarDatos() {
  ...
  try {
    ...
    if (filasResultados.length > 0) {
      promesas.push(agregarFilasASheet('Resultados', filasResultados));
    }
    
    if (filasFotos.length > 0) {
      promesas.push(agregarFilasASheet('Fotos', filasFotos));
    }
    ...
    promesas.push(agregarFilasASheet('Log', filaLog));
  ...
  try {
    ...
    if (fotosData && fotosData.length > 0) {
      fotosData.forEach(f => {
        ...
        const url = (f.url_foto || f.url || '').trim();
